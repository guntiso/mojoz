...
trait I18nRules {
  def setI18n[T](md: TableMetadata[_], view: ViewDef[T]): ViewDef[T]
}

private[in] class NoI18n extends I18nRules {
  override def setI18n[T](md: TableMetadata[_], t: ViewDef[T]) = t
}
...
private[in] class SuffixI18nRules(
  ...
  override def setI18n[T](md: TableMetadata[_], t: ViewDef[T]) = {
    ...
    val i18n = t.fields.filter(!_.isExpression).filter(!_.isCollection)
      ...
      .filter(f => i18nSuffixes.forall(sx =>
        md.tableDef(f.table).cols.exists(_.name == f.name + sx)))
      .toSet
      .filter(f =>
